...
const ProfileTimeline = ({ timeline }) => (
  <div className="space-y-10">
    {timeline.map((block, idx) => (
      <div key={idx}>
        <div className="sticky top-0 bg-background py-1 mb-4">
          <span className="text-xs font-bold bg-primary/10 text-primary px-3 py-1 rounded-full shadow-sm">
            {block.date}
          </span>
        </div>
        <ol className="relative text-gray-500 border-s border-gray-200 dark:border-gray-700 dark:text-gray-400 ml-6">
          {block.items.map((item, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`absolute flex items-center justify-center w-8 h-8 -start-4 ring-4 ring-white dark:ring-gray-900  ${
                    active
