...
const CalendarWidget = () => {
  ...
  const [externalEvents, setExternalEvents] = useState([
    ...
    {
      title: "Do homework",
      color: COLORS[4].className,
      fcColor: COLORS[4].value
    },
    {
      title: "Work on UI design",
      color: COLORS[0].className,
      fcColor: COLORS[0].value
    },
    {
      title: "Sleep tight",
      color: COLORS[3].className,
      fcColor: COLORS[3].value
    }
  ])
  ...
  const handleEventReceive = info => {
    const isDuplicate = events.some(
      event =>
        event.title === info.event.title && event.start === info.event.start
    )
    ...
    if (!isDuplicate) {
      ...
      if (removeAfterDrop) {
        setExternalEvents(prev =>
          prev.filter(e => e.title !== info.event.title)
        )
      }
    }
  }
  ...
  const handleAddExternalEvent = () => {
    if (newEventTitle.trim()) {
      setExternalEvents([
        ...
        {
          title: newEventTitle,
          color: newEventColor.className,
          fcColor: newEventColor.value
        }
      ])
      setNewEventTitle("")
    }
        ...
        <Card className="flex flex-col gap-2 p-4">
          ...
          <div className="mt-2 mb-2 flex items-center">
            <Checkbox
              ...
              onCheckedChange={checked => setRemoveAfterDrop(checked)}
              className="mr-2"
            />
